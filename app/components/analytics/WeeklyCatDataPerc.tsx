...
const Example: React.FC<ExampleProps> = ({ analyticsData }) => {
  ...
  useEffect(() => {
    ...
    const dogTriggers = dataArray.filter(
      (item) => item.trigger_type.toLowerCase() === "cat"
    );

    if (dogTriggers.length === 0) {
      console.log("No cat trigger data found");
      return;
    }
    ...
    const option: echarts.EChartsOption = {
      title: {
        text: "Weekly Cat Trigger Reaction Percentages",
        left: "center",
      },
      tooltip: {
        trigger: "axis",
        ...
        formatter: function (params: any) {
          ...
          result += `<strong>Total Cat Encounters: ${total}</strong>`;
          return result;
        },
      },
      legend: {
